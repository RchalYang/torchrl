...
from torchrl.networks import ZeroNet
...
from torchrl.algo import Reinforce
...
def experiment(args):
    ...
    #     ZeroNet(), env = env, pf = pf, replay_buffer = replay_buffer, device = "cuda", 
    #     train_render=False
    # )

    params['general_setting']['collector'] = ParallelOnPlicyCollector(
        ZeroNet(), env = env, pf = pf, replay_buffer = replay_buffer, device=device, worker_nums=2
    )
