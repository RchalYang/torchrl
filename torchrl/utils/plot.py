...
for eachcolor, eachlinestyle, exp_name in zip(
    ...
    for seed in args.seed:
        ...
        for e in summary_iterator(
            get_name(os.path.join(args.log_dir, exp_name, file_path))
        ):
    ...
    plt.plot(
        final_step, all_mean, label=exp_name,
        color=eachcolor, linestyle=eachlinestyle, linewidth=1
    )
    plt.plot(
        final_step, all_upper, color=eachcolor,
        linestyle=eachlinestyle, alpha=0.23, linewidth=0.5
    )
    plt.plot(
        final_step, all_lower, color=eachcolor,
        linestyle=eachlinestyle, alpha=0.23, linewidth=0.5
    )
    plt.fill_between(
        final_step, all_lower, all_upper, facecolor=eachcolor, alpha=0.2
    )
